...
#define EXDIS_SERVICE_JSON_KEY "exdis"
...
namespace exdisservice
{
    ...
    struct ExdisServiceConfiguration
    {
        string name;
        string requestGatewayTopic;
        string responseGatewayTopic;
        string requestServiceTopic;
        string responseServiceTopic;
        string readingServiceTopic;
        map<string, string> requestTopicMap;
        ...
        void loadConfig(string filename)
        {
            ...
            std::string macAddr = osExec("cat /sys/class/net/eth0/address");
            macAddr.erase(std::remove(macAddr.begin(), macAddr.end(), ':'), macAddr.end());
            macAddr.erase(std::remove(macAddr.begin(), macAddr.end(), '\n'), macAddr.end());
            this->requestGatewayTopic = macAddr + "host";
            this->responseGatewayTopic = macAddr + "gateway";
            this->responseTopicMap = data.value("client-response-topics", map<string, string>{});

            if (data.contains(EXDIS_SERVICE_JSON_KEY))
            {
                auto transport = data[EXDIS_SERVICE_JSON_KEY].get<json>();
                this->name = transport.value("name", EXDIS_SERVICE_JSON_KEY);
                this->responseServiceTopic = transport.value("telemetrytopic", "");
                this->readingServiceTopic = transport.value("responsetopic", "");
            }
        }
